...
ANALYTE_CONFIG = {
    "chlorine": {
        ...
        "unit":      "mg/L",
        "threshold": None,
        "use_r_correction": True,   # trained on R-corrected G values
    },
    "arsenic": {
        "csv":       "raw_arsenic_data.csv",
        "unit":      "ppb",
        "threshold": 10.0,
        "use_r_correction": False,  # trained on RAW G values (see rgb_extraction_final.py)
    },
}
...
class ColorimetryDataset:
    ...
    def __init__(self, path: str, analyte: str, unit: str, threshold: float | None = None,
                 use_r_correction: bool = True):
        self.path             = path
        self.analyte          = analyte
        self.unit             = unit
        self.threshold        = threshold
        self.use_r_correction = use_r_correction
...
datasets: dict[str, ColorimetryDataset] = {
    name: ColorimetryDataset(
        ...
        use_r_correction=cfg["use_r_correction"],
    )
...
def analyze():
    ...
    # ─────────────── CORE LOGIC ───────────────
    if dataset.use_r_correction:
        offset = 255 - r
        corrected_g = g + offset
    else:
        # This analyte's model was trained on raw G values — don't correct.
        offset = 0
        corrected_g = g

    logger.info(
        f"[{analyte}] RGB=({r},{g},{b}) | r_correction={dataset.use_r_correction} | "
        f"offset={offset} | model_input_g={corrected_g}"
    )
